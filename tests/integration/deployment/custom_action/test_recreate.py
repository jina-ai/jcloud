...
def test_recreate_deployment():
    with CloudDeployment(
        ...
        deployment._loop.run_until_complete(deployment._terminate())
        assert utils.eventually_reaches_phase(deployment, Phase.Deleted)

        # recreate the deployment
        deployment._loop.run_until_complete(deployment.recreate())
        assert utils.eventually_reaches_phase(deployment, Phase.Serving)
