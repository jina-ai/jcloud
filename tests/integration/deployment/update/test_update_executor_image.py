...
new_exc_image_deployment_file = 'update_image.yml'
protocol = 'grpc'
...
def test_update_executor_image():
    with CloudDeployment(
        ...
        deployment.path = os.path.join(deployments_dir, new_exc_image_deployment_file)
        ...
        assert (
            get_dict_list_key_path(status, ['spec', 'with', 'uses'])
